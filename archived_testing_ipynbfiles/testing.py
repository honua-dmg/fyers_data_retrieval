""" 
from data import Depth,Symbol
import time

symbol_access = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..XkKx8VcIvFu2yPAY2WeC_vYHJ1f3KSrcnmneVbjUVv4'
depth_access = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..QypnBnF9c5MEWCWnYB7V6HzuR5o9R0UM9SHAKozNmoY'

depth = Depth(depth_access,['NSE:SBIN-EQ','BSE:SBIN-A','NSE:TATAMOTORS-EQ'])
symbol = Symbol(symbol_access,['NSE:SBIN-EQ','BSE:SBIN-A','NSE:TATAMOTORS-EQ'])

depth.connect()
symbol.connect()

depth.subscribe()
symbol.subscribe()

time.sleep(10)
depth.unsubscribe()
symbol.unsubscribe()"""
import threading
from data import Depth,Symbol
import time
def process(class_type,auth_code,stonks,wait_time):
    access = auth_code
    data = class_type(access,stonks)
    data.connect()
    data.subscribe()
    time.sleep(wait_time)
    data.unsubscribe()
dep_auth = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..QypnBnF9c5MEWCWnYB7V6HzuR5o9R0UM9SHAKozNmoY'
sym_auth = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..XkKx8VcIvFu2yPAY2WeC_vYHJ1f3KSrcnmneVbjUVv4'
stonks = ['NSE:SBIN-EQ','BSE:SBIN-A','NSE:TATAMOTORS-EQ']
wait_time = 10
dep_thread = threading.Thread(target=process, args=(Depth, dep_auth, stonks, wait_time))
symb_thread = threading.Thread(target=process, args=(Symbol, sym_auth, stonks, wait_time))
dep_thread.start()
symb_thread.start()